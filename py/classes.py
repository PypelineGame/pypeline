...
class Player(Entity):
    ...
    def __init__(self, x, y):
        ...
        self.strong_attack_timer = 90
        self.count_strong_attack = 0
        ...
        self.frame_counter, self.counter, self.jump_counter, self.melee_counter = 0, 0, 0, 0
    ...
    def update(self, up, down, left, right, running, platforms, enemies, enemy_sprites, bullets, camera, collision_blocks, RESET_LEVEL_FLAG, entities, blocks):
        ...
        if self.strong_attack or self.melee_attack:
            ...
            if not self.strong_attack:
                self.attack_box.y = self.rect.y
                if self.melee_counter < PLAYER_MAX_ATTACK_FRAMES * 1:
                    pass#self.melee_trigger = False
            ...
            self.melee_counter += 1
            if self.melee_counter >= PLAYER_MAX_ATTACK_FRAMES * 5:
                self.melee_trigger = True
...
class RedGhost(Ghost):
    def __init__(self, x, y):
        Ghost.__init__(self, x, y)
        self.image = pygame.image.load(SPRITES_DIRECTORY + 'ghosts/boo/normal/red/1.png')
